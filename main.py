...
def create_team(data: TeamCreate):
    ...
    c = conn.cursor()
    try:
        c.execute("INSERT INTO team (name) VALUES (?)", (data.name,))
        conn.commit()
...
def delete_team(team_id: int):
    ...
    c = conn.cursor()
    # 팀에 소속된 직원이 있는지 확인
    c.execute("SELECT COUNT(*) as cnt FROM user WHERE team_id = ?", (team_id,))
    count = c.fetchone()["cnt"]
    if count > 0:
        raise HTTPException(status_code=400, detail=f"이 팀에 {count}명의 직원이 있어 삭제할 수 없습니다")
    
    c.execute("DELETE FROM team WHERE id = ?", (team_id,))
    conn.commit()
    return {"success": True}
...
def get_team_status(team_id: int, date: str = None):
    ...
    # 팀원 목록 (관리자 제외)
    c.execute("SELECT id, name FROM user WHERE team_id = ? AND role != 'admin'", (team_id,))
    members = c.fetchall()
...
def request_leave(data: LeaveRequest):
    ...
    # 잔여 연차 확인
    c.execute("SELECT annual_leave_total, annual_leave_used FROM user WHERE id = ?", (data.user_id,))
    user = c.fetchone()
...
def cancel_leave(leave_id: int):
    ...
    # 휴가 정보 가져오기
    c.execute("SELECT user_id, type FROM leave WHERE id = ?", (leave_id,))
    leave = c.fetchone()
    ...
    c.execute("DELETE FROM leave WHERE id = ?", (leave_id,))
    c.execute(
        "UPDATE user SET annual_leave_used = annual_leave_used - ? WHERE id = ?",
